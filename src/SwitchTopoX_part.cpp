...
SwitchTopoX_part::SwitchTopoX_part(const Topology *topo_in, const Topology *topo_out, const int shift[3], H3LPR::Profiler *prof)
...
void SwitchTopoX_part::setup_buffers(opt_double_ptr sendData, opt_double_ptr recvData) {
    BEGIN_FUNC;
    ...
    auto opinit = [=](const int nchunks,
        ...
        for (int ir = 0; ir < nchunks; ++ir) {
            ...
            size_t         count  = cchunk->size_padded;
            size_t         n_part = cchunk->nda;
            FLUPS_CHECK(n_part < std::numeric_limits<int>::max(), "message is too big: %ld vs %d", n_part, std::numeric_limits<int>::max());
            ...
            MPI_Info info;
            MPI_Info_create(&info);
            MPI_Precv_init(buf, (int)n_part, (int)(count), MPI_DOUBLE, cchunk->dest_rank, cchunk->dest_rank, cchunk->comm, info, recv_rqst + ichunk);
            MPI_Info_free(&info);
...
#else
            send_order[prior_idx[0]] = ichunk;
            MPI_Psend_init(buf,(int)(n_part),(int)(count), MPI_DOUBLE, cchunk->dest_rank, send_tag, cchunk->comm, info,send_rqst + prior_idx[0]);
...
SwitchTopoX_part::~SwitchTopoX_part(){
    BEGIN_FUNC;
    //--------------------------------------------------------------------------
 ...
 * @param sign
 */
void SwitchTopoX_part::execute(opt_double_ptr v, const int sign) const {
    BEGIN_FUNC;
    //--------------------------------------------------------------------------
...
void SwitchTopoX_part::disp() const {
    BEGIN_FUNC;
    FLUPS_INFO("------------------------------------------");
