...
SwitchTopoX_stream::SwitchTopoX_stream(const Topology *topo_in, const Topology *topo_out, const int shift[3], H3LPR::Profiler *prof)
...
/** return the stream id for the current thread id
 */
static int get_stream(const int nstream) {
    const int ithread = omp_get_thread_num();
    FLUPS_CHECK(nstream >= ithread, "the thread id is bigger than the number of available streams");
    return ithread;
}

void SwitchTopoX_stream::setup_buffers(opt_double_ptr sendData, opt_double_ptr recvData) {
    BEGIN_FUNC;
    ...
    //..........................................................................
    // create as many streams as required, one per thread is required
    n_comm_  = omp_get_max_threads();
    streams_ = reinterpret_cast<MPIX_Stream *>(m_calloc(n_comm_ * sizeof(MPIX_Stream)));
    comm_    = reinterpret_cast<MPI_Comm *>(m_calloc(n_comm_ * sizeof(MPI_Comm)));

    // prevent the call to MPIX_Stream_create to fail in case of error
    MPI_Errhandler err_handler;
    MPI_Comm_get_errhandler(MPI_COMM_SELF, &err_handler);
    MPI_Comm_set_errhandler(MPI_COMM_SELF, MPI_ERRORS_RETURN);
    n_streams_ = 0;
    for (int i = 0; i < n_comm_; ++i) {
#if (M_FLUPS_STREAM)
        // try to create a stream
        int err = MPIX_Stream_create(MPI_INFO_NULL, streams_ + i);
        // if we have a stream available, use it. if not, dup the comm
        if (err == MPI_SUCCESS) {
            MPIX_Stream_comm_create(subcomm_, streams_[i], comm_ + i);
            n_streams_++;
        } else {
            MPIX_Stream_comm_create(subcomm_, MPIX_STREAM_NULL, comm_ + i);
        }
#else
        MPI_Comm_dup(subcomm_, comm_ + i);
#endif
    }
    // reset the normal behavior
    MPI_Comm_set_errhandler(MPI_COMM_SELF, err_handler);
    FLUPS_INFO("%d comms are using streams (total %d)", n_streams, n_comm_);
...
SwitchTopoX_stream::~SwitchTopoX_stream() {
    BEGIN_FUNC;
    //--------------------------------------------------------------------------
 ...
 * @param sign
 */
void SwitchTopoX_stream::execute(opt_double_ptr v, const int sign) const {
    BEGIN_FUNC;
    //--------------------------------------------------------------------------
...
void SwitchTopoX_stream::disp() const {
    BEGIN_FUNC;
    FLUPS_INFO("------------------------------------------");
